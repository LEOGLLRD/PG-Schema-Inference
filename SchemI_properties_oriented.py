...
from propertiesOriented.preprocessing_properties import pg_to_json, get_edges_card
# Import the MapReduce inference module
import propertiesOriented.mapreduce_properties as mr
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Infer PG schema (Properties-oriented)")
    parser.add_argument('-c', '--config', help="Path to the JSON configuration file", required=True)
    args = parser.parse_args()

    print("\nProperties-oriented PG schema inference (Automated Mode)\n")
    ...
    equiv_properties = mr_config.get("equiv_properties", "l")
    ...
    MR_nodes_file = mr.call_mapreduce(nodes_file, equiv=equiv_properties, driver_memory=spark_driver_memory, master=spark_master)
    schemaNodes = mr.parse_mapreduce_schema(MR_nodes_file)
    if schemaNodes == 'Null': schemaNodes = {}

    # 3.2 Process Edges
    print(" -> Inferring Edges schema...")
    MR_edges_file = mr.call_mapreduce(edges_file, equiv=equiv_properties, driver_memory=spark_driver_memory, master=spark_master)
    ...
    MR_unlab_file = mr.call_mapreduce(unlab_file, equiv=equiv_properties, driver_memory=spark_driver_memory, master=spark_master)
    ...
    schema_output_file = f"schema_{DBname}_properties.json"
