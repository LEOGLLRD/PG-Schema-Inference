...
def Neo4jNode_to_json(labels, props):
    """ Serialize raw node data (labels and properties) into json format. Returns bytes. """
...
def pg_to_json(uri, user, pwd, filename, limitnodes_pct=100, limitedges_pct=100, dbname="neo4j", num_workers=4, fetch_size=25000):
    ...
    with driver.session(database=dbname) as session:
        ...
        nodeProperties = session.execute_read(lambda tx: tx.run("call db.schema.nodeTypeProperties").data())
        ...
        edgeProperties = session.execute_read(lambda tx: tx.run("call db.schema.relTypeProperties").data())
